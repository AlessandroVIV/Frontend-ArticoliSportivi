...
export class DettagliArticoloComponent {
  ...
  onSubmit() {
    const updateBody:any = {id: Number(this.route.snapshot.paramMap.get('id'))}
    ...
    this.backendService.updateArticoloScarpa(updateBody)
      .subscribe((resp:any) => {
  ...
  onDeleteAction() {
    this.backendService.removeArticolo({
      id: this.id
    })
      .subscribe((resp: any) => {
        if (resp.rc) {
